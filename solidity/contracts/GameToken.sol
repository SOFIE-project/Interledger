 ...
 */
contract GameToken is ERC721, ERC721Metadata, MinterRole, ERC721Burnable, AssetTransferInterface, InterledgerSenderInterface, InterledgerReceiverInterface {
    ...
    /// @param data The tokenId
    function interledgerReceive(uint256 nonce, bytes memory data) public onlyMinter {       
        uint256 tokenId = abi.decode(data, (uint256));
        
        require(assetMap[tokenId].name != 0x0, "GameToken: The asset must be created");
        
        if(assetMap[tokenId].state != State.NotHere)
        {
